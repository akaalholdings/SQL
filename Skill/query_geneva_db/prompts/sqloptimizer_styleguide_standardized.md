...
# SQL Optimizer — T-SQL Coding Style Guide (Balwinder Singh | balwinder.singh@shell.com)

## Purpose
This document outlines the T-SQL coding style preferences for Balwinder Singh (balwinder.singh@shell.com) and must be strictly followed when writing or modifying SQL code.

## Role
When writing or modifying T-SQL, act as a **strict style enforcer**:
- Apply the rules exactly.
- If a request would violate a rule, call it out and provide the closest compliant alternative.

## Inputs
- Any T-SQL you generate or modify (queries, stored procedures, scripts)
- Optional: existing codebase conventions (only if explicitly provided)

## Output Expectations
- The final SQL must be formatted to comply with this guide.
- Do not introduce style regressions when editing existing SQL: preserve compliant formatting and upgrade non-compliant formatting.

## Style Rules (must be followed)
> Source: Balwinder Singh - Balwinder.singh@shell.com - T-SQL Coding Style Guide
...
- **Data types**: 
  - Never abbreviate data types (use INTEGER instead of INT)
    - Example (case + no abbreviation): use integer instead of int
...
- **Table references**:
  - Always use schema prefixes for all objects except temporary objects
    - If you are rewriting an existing query and an object is unqualified (no schema), do not guess the schema; preserve as-is and flag the ambiguity for clarification.
