...
DB_ENVIRONMENTS = {
    "mid": {
        "server": "shellgeneva-analytics.database.windows.net",
        "database": "mid",
        "description": "Replica of mid PROD, read-only",
        "type": "SQL Server",
        "production": False,
    },
    "mid_prod": {
        "server": "shellgeneva.database.windows.net",
        "database": "mid",
        "description": "Production DB (read-only caution: HUGE WARNING)",
        "type": "SQL Server",
        "production": True,
    },
    "mid_preprod": {
        "server": "shellgenevapreprod.database.windows.net",
        "database": "mid_preprod",
        "description": "Pre-production replica - we make a snapshot of Prod once a week",
        "type": "SQL Server",
        "production": False,
    },
    "mid_test": {
        "server": "gnvatst.database.windows.net",
        "database": "mid_test",
    ...
    "mid_sandbox": {
        "server": "gnvadev.database.windows.net",
        "database": "mid_sandbox",
        "description": "Sandbox used by UI/API",
        "type": "SQL Server",
        "production": False,
    },
    "mid_dev": {
        "server": "gnvadev.database.windows.net",
        "database": "mid_dev",
        "description": "Dev environment used by pipelines",
        "type": "SQL Server",
        "production": False,
    },
    "synapse_dev": {
        "server": "shell-01-eun-sq-jugjhnpwhhmrodlueolg.database.windows.net",
        "database": "shell-01-eun-sqdw-zhmlmkezgutgtabfdenp",
        "description": "DNA platform Synapse dev",
        "type": "Synapse",
        "production": False,
    },
    "synapse_test": {
        "server": "shell-01-eun-sq-galgbinffyymqwkukwfl.database.windows.net",
        "database": "shell-01-eun-sqdw-xbgjtwbxroucfezymxvb",
        "description": "DNA platform Synapse test",
        "type": "Synapse",
        "production": False,
    },
    "synapse_prod": {
        "server": "shell-31-eun-sq-atgefziwobxvrcknmhjq.database.windows.net",
        "database": "shell-31-eun-sqdw-sltkkvigofnzsxruvzbq",
        "description": "DNA platform Synapse prod",
        "type": "Synapse",
        "production": True,
    },
    "uc_dev": {
        "server_hostname": "adb-3856895995337257.17.azuredatabricks.net",
        "http_path": "/sql/1.0/warehouses/d9d27d8edb4ff825",
        "catalog": "",
        "schema": "",
        "http_path_env": ("QUERY_GENEVA_DB_UC_HTTP_PATH", "DATABRICKS_HTTP_PATH"),
        "catalog_env": ("QUERY_GENEVA_DB_UC_CATALOG", "DATABRICKS_CATALOG"),
        "schema_env": ("QUERY_GENEVA_DB_UC_SCHEMA", "DATABRICKS_SCHEMA"),
        "description": "EDP Databricks dev workspace",
        "type": "Databricks",
        "production": False,
    },
    "uc_test": {
        "server_hostname": "adb-1275967750467369.9.azuredatabricks.net",
        "http_path": "/sql/1.0/warehouses/cc46d81a37a2aa72",
        "catalog": "",
        "schema": "",
        "http_path_env": ("QUERY_GENEVA_DB_UC_HTTP_PATH", "DATABRICKS_HTTP_PATH"),
        "catalog_env": ("QUERY_GENEVA_DB_UC_CATALOG", "DATABRICKS_CATALOG"),
        "schema_env": ("QUERY_GENEVA_DB_UC_SCHEMA", "DATABRICKS_SCHEMA"),
        "description": "EDP Databricks test workspace",
        "type": "Databricks",
        "production": False,
    },
    "uc_preprod": {
        "server_hostname": "adb-1677585743409209.9.azuredatabricks.net",
        "http_path": "/sql/1.0/warehouses/aa3db777ad19955b",
        "catalog": "",
        "schema": "",
        "http_path_env": ("QUERY_GENEVA_DB_UC_HTTP_PATH", "DATABRICKS_HTTP_PATH"),
        "catalog_env": ("QUERY_GENEVA_DB_UC_CATALOG", "DATABRICKS_CATALOG"),
        "schema_env": ("QUERY_GENEVA_DB_UC_SCHEMA", "DATABRICKS_SCHEMA"),
        "description": "EDP Databricks pre-production workspace",
        "type": "Databricks",
        "production": False,
    },
    "uc_prod": {
        "server_hostname": "adb-4018826223110734.14.azuredatabricks.net",
        "http_path": "/sql/1.0/warehouses/3a939c17c170c4f6",
        "catalog": "",
        "schema": "",
        "http_path_env": ("QUERY_GENEVA_DB_UC_HTTP_PATH", "DATABRICKS_HTTP_PATH"),
        "catalog_env": ("QUERY_GENEVA_DB_UC_CATALOG", "DATABRICKS_CATALOG"),
        "schema_env": ("QUERY_GENEVA_DB_UC_SCHEMA", "DATABRICKS_SCHEMA"),
        "description": "EDP Databricks production workspace",
        "type": "Databricks",
        "production": True,
    },
}
...
def _env_first(names: Any) -> Optional[str]:
    if not names:
        return None
...
def _setting(env: dict, key: str) -> Optional[str]:
    return env.get(key) or _env_first(env.get(f"{key}_env"))
...
def get_db_connection(db_alias: str, timeout_seconds: int):
    ...
    if env.get("type") == "Databricks":
        ...
        http_path = _setting(env, "http_path")
        if not http_path:
            names = env.get("http_path_env") or ("DATABRICKS_HTTP_PATH",)
            if isinstance(names, str):
                names = (names,)
            raise RuntimeError(
                f"Databricks alias '{db_alias}' needs a SQL warehouse HTTP path. "
                f"Set one of: {', '.join(names)}. "
                "In Databricks, open SQL Warehouses -> your warehouse -> Connection details -> HTTP Path."
            )
        ...
        catalog = _setting(env, "catalog")
        schema = _setting(env, "schema")
        if catalog:
...
def _fetch_shape_databricks(cursor, alias: str, table_ref: str, parts: list[str], env: dict) -> dict:
    ...
    if len(parts) == 3:
        ...
        table = parts[0]
    catalog = catalog or _setting(env, "catalog")
    schema = schema or _setting(env, "schema")
    if not schema:
        raise RuntimeError(
            ...
            columns.append(
                {
                    ...
                    "normalized": _normalize_type("databricks", data_type),
                    "nullable": True,
                }
            )
        if not columns:
...
def main() -> None:
    ...
    for alias in involved:
        ...
        is_production = bool(env.get("production", False))
        if is_production:
