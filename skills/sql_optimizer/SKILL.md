...
7. **Reconcile:** after each result call `get_tuning_session`; preserve every candidate, returned evidence id, terminal state, metric provenance, plan delta, equivalence result, execution count, and continuation flag. Retain the complete benchmark request and exact idempotency key until `get_tuning_session` reconciles its evidence. Retrieve a lost response there rather than rerunning it with a new key. A `completed` or `cancelled` session is retrieval-only: do not submit new or replayed benchmark work.
